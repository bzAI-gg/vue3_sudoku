<template>
  <div class="sudoku-home">
    <div class="headbox">
      ...
      ></my-timer>
      <div class="copy-bnt" title="复制数独字符串" @click="copy">复制</div>
    </div>
    <div ref="sudokubox">
      <sudoku
        :arr="arr"
    ...
    <num-keys
      @numDown="handleNumInput"
      @eraseDown="handleErase"
    ...
    <div v-if="showWin" class="win">
      ...
      <div class="minibox">
        <mini-sudoku
          class="mini"
          :sudoku-array="sudoku_tool.stringToSudokuArray(route.query.arrstr)"
        ></mini-sudoku>
...
import { sudoku_tool, dateTime } from "../utils/util";
...
const copy = () => {
  const text = sudoku_tool.sudokuArrayToString(
    arr.value.map((item) => item.map((cell) => cell.value))
  );
...
const formatTime = (elapsed) => {
  ...
  const milliseconds = Math.floor(diff.milliseconds / 10)
    .toString()
    .padStart(2, "0");
...
onMounted(() => {
  arr.value = sudoku_tool.normalised(
    sudoku_tool.stringToSudokuArray(route.query.arrstr)
  );
