<template>
  <div class="sudoku-home">
      ...
      <div class="input-item">
        ...
        <div class="input-d" :class="showInput ? '' : 'input-d-close'">
          <textarea
            ...
            placeholder="输入81位数字，0表示空格"
          />
          <div class="input-bnt my_unselectable" @click="loadFromString">
            加载
          </div>
        </div>
      </div>
    ...
    <div class="aside-bnt" @click="changeStepShow">
      {{ showStep ? ">" : "<" }}
    </div>
    <!-- 破解步骤显示 -->
    <div class="step-box" :class="showStep ? '' : 'step-box-hid'">
      ...
      <div class="step-goup-steps">
        <div
          v-for="(item, index) in steps[currentGroup]"
          :key="index"
          class="step-item"
          :class="{
            'current-step':
              currentStep.group == currentGroup && currentStep.index == index,
          }"
        ...
        >
          ...
          <span v-else-if="item.txt == '区'" class="step-try try-h"
            >唯一候选</span
          >
          <span v-else-if="item.txt == '试'" class="step-try try-c"
            >尝试填入</span
          >
        </div>
      </div>
    ...
    <div class="ctrl-box">
      ...
      <button
        ...
        class="ctrl-btn"
        @click="
          fillBySteps(steps.length - 1, steps[steps.length - 1].length - 1)
        "
      ...
      <button
        v-if="steps.length > 0"
        class="ctrl-btn"
        @click="fillBySteps(-1, -1)"
        :disabled="
          !arr.some((item) =>
            item.some((cell) => !cell.fixed && cell.value !== 0)
          )
        "
      >
        移除
      ...
      <button
        ...
        :disabled="
          ...
          currentStep.index === steps[steps.length - 1].length - 1
        "
      >
        下一步
      </button>
...
import { sudoku_tool } from "../utils/util";
...
const initByStr = (str) => {
  arr.value = sudoku_tool.normalised(
    sudoku_tool.stringToSudokuArray(sudoku_tool.formatSudokuStr(str))
  );
...
const solve = () => {
  ...
  sudokuString.value = sudoku_tool.sudokuArrayToString(
    arr.value.map((item) => item.map((cell) => cell.value))
  );
  initByStr(sudokuString.value);
  ...
  if (pj) {
    ...
    pj.step.forEach((item) => {
      ...
      arr.value[item.y][item.x].try =
        item.txt == "单" ? 0 : item.txt == "区" ? 1 : 2;
    });
...
const nextStep = () => {
  if (
    currentStep.value.index ==
    steps.value[currentStep.value.group].length - 1
  ) {
